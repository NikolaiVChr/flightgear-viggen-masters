...
\footnotetext{%
  Despite the name, IFF can by no mean distinguish an enemy aircraft from
  e.g.\ a civilian aircraft, or an aircraft with a non-functioning IFF system.
}
...
\begin{description}
  ...
  \item[\keys{N}] \ifbool{AJS}{Select next radar track.}{Select/click with cursor}
...
\JAonly{\item[\keys{\ctrl+F}] IFF contact under MI cursor.}
\end{description}

\paragraph{Combat}
\begin{description}
\JAonly{\item[\keys{H}] Toggle master arm.}
...
\begin{itemize}
  \item Enable the option \menu{\ifbool{AJS}{AJS-37}{JA-37Di}>Options>Arrow keys control radar cursor},
    to use the arrow keys to move the cursor, and \keys{\return} to click/select.
...
/controls/display/cursor-click\end{alltt}
