...
\sidepicture{
  \paragraph{Chronometer \ifbool{AJS}{(\cockpitref{fig:front-panel}{item:clock})}{(\cockpitref{fig:right-panel}{item:clock})}}
...
\begin{description}
  ...
  \item[Auto throttle] Steady: normal auto-throttle disengagement.
    ...
    Pull auto-throttle lever to off (up) position to reset.
  \item[No reverse] Failure of thrust reverse or tertiary air intake (blinking).
...
\ifbool{AJS}{%
  The FR 24 radio panel (\cockpitref{fig:left-panel}{item:fr29}) is used to set
  ...
  The FR 29 radio panel (\cockpitref{fig:left-panel}{item:fr29}) is used to set 
...
(\cockpitref{fig:fr29}{item:knob-mode}) are the following.
\begin{description}
...
\ifbool{AJS}{
  ...
  FR 22 channel selector (\cockpitref{fig:left-panel}{item:fr22})
  and frequency selector (\cockpitref{fig:front-panel}{item:frequency-sel}).
  ...
  To directly select a frequency, press button `--' (\cockpitref{fig:fr22}{item:channel-freq})
  ...
  press the corresponding button (\cockpitref{fig:fr22}{item:channel-guard} or
  \cockpitref{fig:fr22}{item:channel-special}).

  To select an airbase channel, set the airbase number with the airbase knob (\cockpitref{fig:fr22}{item:knob-base})
  and select the channel with one of the airbase channel buttons (\cockpitref{fig:fr22}{item:channel-base}).
  For instance, set the airbase number to 21 and press button A/G for channel 21A.

  To select one of the global channels E,F,G, set the airbase knob (\cockpitref{fig:fr22}{item:knob-base})
  to one of the positions ALLM (every 6th position), and select the channel
  with one of buttons A/G, C/F, D/E (\cockpitref{fig:fr22}{item:channel-base}).

  To select a generic channel, set the channel group with the group knob (\cockpitref{fig:fr22}{item:knob-group})
  and select the channel with one of the channel buttons (\cockpitref{fig:fr22}{item:channel-group}).
  For instance, set the group number to 06 and press button 2 for channel 062.
}{
  \subsection{KV1 and KV3 Channel Selectors}
  The KV1 (\cockpitref{fig:left-panel}{item:kv1}) is used to set the channel of frequency for the FR 29 comm radio.
  The KV3 (\cockpitref{fig:left-panel}{item:kv3}) is used to set the datalink channel.
  ...
  (\cockpitref{fig:kv1}{item:kv3-display,item:kv1-base-screen,item:kv1-nr-screen,item:kv1-mhz-screen}),
  press the corresponding `clear' button and input the new value using the KV1 keypad
  (\cockpitref{fig:kv1}{item:kv1-keypad}).
  ...
  (\cockpitref{fig:kv1}{item:kv1-base-button,item:kv1-nr-button,item:kv1-mhz-button}).
  ...
  \subsection{FR 31 Radio Panel (\cockpitref{fig:right-panel}{item:fr31})}
  \begin{figure}[h]
    \centering
  ...
  from the current mode, (\cockpitref{fig:fr31}{item:base,item:nr,item:mhz}),
  then press the `clear' button and enter the new value with the keypad (\cockpitref{fig:fr31}{item:keypad}).
...
\JAonly{
  ...
  This information is displayed on the TI (\cockpitref{fig:front-panel}{item:TI}).

  Datalink is activated with the DL button in the SYST page on the TI.
  The KV3 screen (\cockpitref{fig:kv1}{item:kv3-display}) is used to set the datalink channel
  ...
  provided each aircraft should set it to a different number, (e.g.\ its position in the formation).
  ...
  Aircraft A may also transmit an IFF information for aircraft B.
  The symbol for B is green if A has identified B as friendly, red if it has identified B as hostile,
  and yellow if no IFF information is transmitted.
}
...
Additionally, pressing \keys{\ctrl+F} sends an IFF interrogation
to the radar contact on which the MI cursor is pointing.
If it is identified as friendly, a cross is displayed over this contact on the MI
...
The weapon panel (\cockpitref{fig:right-panel}{item:wpn-panel})
...
The first (\cockpitref{fig:wpn-panel}{item:jett-payload}) jettisons all external load, except for the outer pylons.
The second (\cockpitref{fig:wpn-panel}{item:jett-tank}) jettisons only the external fuel tank.

\paragraph{Weapon Selector}
The weapon selection knob (\cockpitref{fig:wpn-panel}{item:wpn-sel}) has the following positions.
\begin{description}
...
\begin{itemize}
  ...
  \item Position IR-RB is an exception: different types of sidewinders can be loaded together,
    ...
    Use button IR-RB FRAMSTEGN (\cockpitref{fig:left-panel}{item:ir-framstegn}) to cycle pylons.
...
This knob (\cockpitref{fig:wpn-panel}{item:dist-sel}) is used to set the bomb release interval,
...
Switch \cockpitref{fig:wpn-panel}{item:impuls} has different functions depending on the selected weapon.
\begin{description}
...
\ifbool{AJS}{
  ...
  \item[\keys{I}] Display target type/callsign on MI.
...
\label{sec:loadout}
While the AJS 37 has access to a wide variety of weapons,
it is restricted in which weapons can be loaded at the same time:
\begin{itemize}
  \item \label{rule:unique} Only one type of primary weapon can be loaded at any time.
  \item Primary weapons are everything except A/A IR missiles (Rb 24, Rb 24J, and Rb 74 Sidewinders).
    Any combination of A/A IR missiles can be loaded together in addition to the primary weapons.
...
\begin{enumerate}
  \item Select weapon type with
    \ifbool{AJS}{
      the weapon selector knob (\keys{C} / \keys{\shift+C}),
      or press \keys{U} for A/A missiles quick selection.
    }{
      \keys{C} (A/A missiles), \keys{\shift+C} (cannon), or through the TI display.
    }
  \ifbool{AJS}{
  \item Set main mode selector to ANF/CBT
    (\cockpitref{fig:left-panel}{item:main-mode}, \keys{M} / \keys{\shift+M}).
  }{
