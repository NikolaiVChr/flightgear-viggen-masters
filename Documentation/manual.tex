...
\begin{description}
  \item[\keys{R}] Toggle radar. \JAonly{In aiming mode: turn radar off.}
  \item[\keys{[} / \keys{]}] Decrease/increase radar range (positions: 15km, 30km, 60km, 120km).
\JAonly{
  \item[\keys{F}] Wide scan mode.
  \item[\keys{\shift+F}] TWS (Track While Scan) mode.
  \item[\keys{\ctrl+F}] STT (Single Target Track) mode.
    Only used in TWS with a primary target selected.
