...
\JAonly{
  \item[\keys{L}] Cursor click. See also \cref{sec:cursor}.
...
\AJSonly{%
  It is also used to control the Rb 75 seeker and to fly the Rb 05A remote controlled missile.\footnote{%
    In the real aircraft, the Rb 05A used a separate sidestick.
    In FlightGear the same controls are reused for simplicity.
  }
}

\AJSonly{\paragraph{cursor movements}}
...
\AJSonly{
\paragraph{Two-stage trigger}
The AJS radar stick trigger has two stages. The positions are called
\begin{itemize}[noitemsep]
  \item T0 (released)
  \item T1 (1st stage)
  \item TV (fully pressed)
\end{itemize}
The idea is that whenever the cursor is used, the pilot holds T1. TV actually 'clicks'.
For example:
\begin{itemize}
  \item For a waypoint fix, holding T1 enters fix mode, releasing it cancels,
    pressing to TV confirms the new position (see \cref{sec:target-wpt}).
  \item For the Rb 75, holding T1 allows to move the seeker,
    releasing it recenters, pressing TV locks (see \cref{sec:rb75}).
\end{itemize}

In the FlightGear model, the corresponding controls are
\begin{itemize}[noitemsep]
  \item \keys{O} for T1
  \item \keys{\shift+O} to toggle \emph{holding} T1. A reminder message is displayed when using this.
  \item \keys{L} for TV (alternatively \keys{\return} with the \menu{AJS-37 > Options > Arrow keys control radar cursor} option).
\end{itemize}
The typical use, for example for a waypoint fix is:
\begin{enumerate}[noitemsep]
  \item Press \keys{\shift+O} (hold T1) to enter fix mode,
  \item adjust the waypoint position by moving the cursor,
  \item press \keys{L} (press TV) to confirm the new position,
  \item leave fix mode by pressing either \keys{O} or \keys{\shift+O} (release T1).
\end{enumerate}

For joystick bindings, the properties are
\begin{alltt}
/controls/displays/cursor-click-stage1
