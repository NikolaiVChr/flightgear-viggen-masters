...
\ifbool{AJS}{
  ...
  \begin{itemize}
    \item The HUD turns on at the earliest 40 seconds after AC power is available.
    \item The MI and TI have up to 2 seconds delay before turning on.
  \end{itemize}
}
  ...
  \begin{multicols}{2}
    \begin{enumerate}[nosep]
      ...
      \item \label{item:rdr-range} Radar ranging mark.
        Indicates that radar is used to compute range to the target.
      \item Side bars, no functionality.
      \item \label{item:distline} Distance line.
        Indicates distance to the target, starting at 8km.
        The side marks indicate optimal firing distance.
        The distance line blinks 2 seconds before optimal firing distance.
      \item \label{item:fire-mark} Firing command indicator.
        Appears 0.5 seconds before optimal firing distance.
      \item \label{item:pullup-bars} Pull-up warning bars.
        Indicates that attack run must be aborted, as safety distance is insufficient.
    \end{enumerate}
  \end{multicols}
...
Aiming mode HUD is used for certain weapons.
It is enabled by either switching to ANF/CBT mode or arming the weapon,
provided that an appropriate weapon type is selected.
It consists in several aiming and ranging indicators (\cref{fig:hud-aiming}),
in addition to the digital altitude.
See \cref{chap:weapons} for more details.
}
