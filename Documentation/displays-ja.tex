...
(\figref{fig:front-panel}{item:TI}).
...
\paragraph{Artificial Horizon (\figref{fig:hud}{item:horizon-hud})}
...
\paragraph{Flight Path Vector (\figref{fig:hud}{item:fpv-hud})}
...
(\figref{fig:hud}{item:dest}) indicates bearing to the next waypoint.

\paragraph{Altitude Scale (\figref{fig:hud}{item:altitude})}
...
\paragraph{Altitude Bars (\figref{fig:hud}{item:altbars})}
...
\paragraph{Airspeed / Mach Indicator (\figref{fig:hud}{item:speed})}
...
\paragraph{Distance Indicator (\figref{fig:hud}{item:distance})}
...
(15.5\textdegree{} if the button \figref{fig:front-panel}{item:autothrottle-lights} is pressed and lit).
...
The MI (\figref{fig:front-panel}{item:MI}) is the JA 37 main radar display.
...
\paragraph{Left side buttons (\figref{fig:mi}{item:buttons})}
...
\paragraph{Right side lights (\figref{fig:mi}{item:lights})}
\begin{description}[nosep]
...
\paragraph{Bottom buttons (\figref{fig:mi}{item:bot-buttons})}
...
\paragraph{%
  Artificial Horizon (\figref{fig:mi-flight}{item:horizon-mi})
  and Flight Path Marker (\figref{fig:mi-flight}{item:fpv-mi})
}
...
Digital altitude (\figref{fig:mi-flight}{item:dig-alt}) is displayed above the artificial horizon.
A marker (\figref{fig:mi-flight}{item:alt-index}) indicates altitude
on the altitude scale (\figref{fig:mi-flight}{item:alt-scale}), on the right side.

Altitude bars (\figref{fig:mi-flight}{item:alt-bars}) are read against the artificial horizon,
...
(\figref{fig:mi-flight}{item:rhm-index}), when the radar altitude is less than 600m.

\paragraph{Heading Indications}
The heading scale (\figref{fig:mi-flight}{item:heading-mi}) shows current heading with a fixed central marker,
and heading to next waypoint with a second marker (\figref{fig:mi-flight}{item:heading-bug}).

\paragraph{Ground Proximity Warning}
The distance between the ground symbol (\figref{fig:mi-flight}{item:ground})
and the flight path marker (\figref{fig:mi-flight}{item:fpv-mi})
...
pointing in the pull-up direction (\figref{fig:mi-flight}{item:gpw-arrow}),
as well as the flashing ground proximity warning light (\figref{fig:mi}{item:gpw}).
...
\paragraph{SIKT / AIM (\figref{fig:mi-text}{item:text-sikt})}
Indicates that the HUD is in aiming mode.

\paragraph{TYST / SILENT (\figref{fig:mi-text}{item:text-tyst})}
Indicates that the radar is off.

\paragraph{Bottom left upper text(\figref{fig:mi-text}{item:text-bl1})}
...
\paragraph{Bottom right upper text (\figref{fig:mi-text}{item:text-br1})}
The following can be displayed, by order of priority:
\begin{enumerate}[nosep]
  \item MREG, indicating a manually registered event (button X3 below the MI \figref{fig:mi}{item:bot-buttons})
  \item FÖ / FAIL, indicating an aircraft failure. Goes off when consulting the TI failure page.
\end{enumerate}

\paragraph{Bottom (\figref{fig:mi-text}{item:text-bl2,item:text-br2})}
The following can be displayed, by order of priority:
\begin{enumerate}[nosep]
  \item Memo for the MI lower buttons (\figref{fig:mi}{item:bot-buttons}),
...
i.e.\ the x-axis represents bearing, read against the heading scale (\figref{fig:mi-radar}{item:head-scale}),
and the y-axis represents distance, read against the distance scale (\figref{fig:mi-radar}{item:dist-scale}).
...
radar scan arc width (\figref{fig:mi-radar}{item:scan-arc}).
On the bottom line, a sweeping cursor indicates current radar azimuth (\figref{fig:mi-radar}{item:sweep}).

Vertically, a thick bar on the altitude scale indicates the range of altitudes
scanned by the radar scale (\figref{fig:mi-radar}{item:vert-scan-arc}).
...
Radar echoes are displayed as trails of small bars (\figref{fig:mi-radar}{item:echo}).
...
After lock, the sides of the square collapse, forming a `\#' (\figref{fig:mi-radar}{item:primary}).
For a secondary target, the symbols are similar,
except the middle of each line is missing (\figref{fig:mi-radar}{item:secondary,item:lost-lock}).
A radar echo with a friendly IFF response is covered with a cross (\figref{fig:mi-radar}{item:friendly}).
...
are shown (\figref{fig:mi-radar}{item:target-info}).
On the altitude scale, a thick horizontal dash indicates target altitude (\figref{fig:mi-radar}{item:tgt-alt}).
A large dot indicates target azimuth and elevation, read relative to the artificial horizon
and flight path marker (\figref{fig:mi-radar}{item:azi-elev}).
...
\begin{description}
  \item[Scan] A wide search mode, with 120\textdegree{} lateral search arc and 12\textdegree{} vertical search arc.
    ...
    Scan mode can be selected with \keys{\shift+F}.
  ...
  \item[TWS (Track While Scan)] A narrow search mode, with 60\textdegree{} lateral search arc and 8\textdegree{} vertical search arc.
    ...
    TWS mode can be selected with \keys{\ctrl+F}.
  \item[Disk search] The radar scans a disk of radius 10\textdegree{} straight ahead,
    ...
    It is selected with \keys{\shift+N}.
\end{description}

\paragraph{Target Designation}
In scan or TWS modes, the MI cursor (\figref{fig:mi-radar}{item:cursor})
...
When the cursor is active, pressing \keys{F} switches between the two cursor modes.

Regardless of cursor mode, designating a contact makes it the primary tracked target,
and causes the MI cursor to disappear.
While a primary target is designated, pressing \keys{F} switches
...
Repeating the previous process in TWS mode allows to track up to four contacts at once:
...
The primary target can be changed with \keys{M}, cycling through all the secondary tracks.
...
the corresponding MI indications (\figref{fig:mi-radar}{item:scan-arc,item:vert-scan-arc}).
...
In aiming mode, SIKT / AIM is displayed at the top of the MI (\figref{fig:mi-text}{item:text-sikt}).
...
and optimistic firing range (\figref{fig:mi-launch}{item:wpn-range}).
...
indicating target range relative to missile performance (\figref{fig:mi-launch}{item:dlz-disk}).
